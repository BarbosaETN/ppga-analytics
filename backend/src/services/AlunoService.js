...
class AlunoService {
    ...
    async criar(dados, options = {}) {
        ...
        if (
            !Number.isInteger(pessoaId) ||
            pessoaId < 1
        ) {
            throw new AppError(
                'pessoa_id deve ser um número inteiro positivo.',
                {
                    statusCode: 400,
        ...
        const pessoa =
            await this.pessoaRepository.findById(
                pessoaId,
                options
            );
        ...
        if (
            ...
            throw new AppError(
                'programa_id deve ser um número inteiro positivo.',
                {
                    statusCode: 400,
                    code: 'VALIDATION_ERROR',
                    details: {
        ...
        const programa =
            await this.programaRepository.findById(
                programaId,
                options
            );
